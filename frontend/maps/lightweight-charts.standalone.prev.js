...
(function () {
    ...
    ***************************************************************************** */
    ...
    var CrosshairMarksPaneView = /** @class */ (function () {
        ...
        CrosshairMarksPaneView.prototype._private__updateImpl = function () {
            ...
            serieses.forEach(function (s, index) {
                ...
                if(!data){
                    console.warn("Florin - No data for CrosshairMarksPaneView");
                    return;
                }
                if (seriesData === null) {
    ...
    function walkDots(ctx, points, lineType, visibleRange) {
        if (points.length === 0) {
            return;
        }
        var x = points[visibleRange.from].x;
        ...
        for (var i = visibleRange.from + 1; i < visibleRange.to; ++i) {
            ...
            ctx.lineTo(currItem.x, currItem.y+100);
        }
    }
    ...
    var PaneRendererDot = /** @class */ (function () {
        function PaneRendererDot() {
            this._data = null;
        }
        PaneRendererDot.prototype.setData = function (data) {
            this._data = data;
        };
        PaneRendererDot.prototype.draw = function (ctx) {
            if (this._data === null || this._data.items.length === 0 || this._data.visibleRange === null) {
                return;
            }
            ctx.lineCap = 'square';
            ...
            for (var i = this._data.visibleRange.from; i < this._data.visibleRange.to; ++i) {
                if(this._data.items[i].order && this._data.items[i].order.orderId == 550){
                    var x = 0;
                }
                ctx.beginPath();
                ctx.arc(this._data.items[i].x, this._data.items[i].y, 4, 0, 2 * Math.PI, false);
                ctx.fillStyle = this._data.lineColor;
                ctx.font = "20px Georgia";
                ctx.fillText("O:" + this._data.items[i].order.orderId, this._data.items[i].x - 10, this._data.items[i].y - 40);
                ...
                ctx.fillStyle = "red";
                ctx.font = "20px Georgia";
                ctx.fillText("P:" + this._data.items[i].y + "$", this._data.items[i].x - 10, this._data.items[i].y - 15);
                ctx.fill();
            }
            // walkDots(ctx, this._data.items, this._data.lineType, this._data.visibleRange);
            // ctx.stroke();
        };
        return PaneRendererDot;
    }());

    var PaneRendererPair = /** @class */ (function () {
        function PaneRendererPair() {
            this._data = null;
        }
        PaneRendererPair.prototype.setData = function (data) {
            this._data = data;
        };
        PaneRendererPair.prototype.draw = function (ctx) {
            if (this._data === null || this._data.items.length === 0 || this._data.visibleRange === null) {
                return;
            }
            ctx.lineCap = 'square';
            ...
            ctx.strokeStyle = this._data.lineColor;
            ctx.lineJoin = 'miter';
            for (var i = this._data.visibleRange.from; i < this._data.visibleRange.to-1; i+=2) {
                if(this._data.items[i].order && this._data.items[i].order.orderId == 3139){
                    var x = 0;
                }
                ctx.beginPath();        
                ctx.moveTo(this._data.items[i].x, this._data.items[i].y);
                ctx.lineTo(this._data.items[i+1].x, this._data.items[i+1].y);
                ctx.stroke();
            }
        };
        return PaneRendererPair;
    }());
    ...
    var DotPaneViewBase = /** @class */ (function (_super) {
        __extends(DotPaneViewBase, _super);
        function DotPaneViewBase(series, model) {
            return _super.call(this, series, model, true) || this;
        }
        DotPaneViewBase.prototype._convertToCoordinates = function (priceScale, timeScale, firstValue) {
            timeScale.indexesToCoordinates(this._items, undefinedIfNull(this._itemsVisibleRange));
            priceScale.pointsArrayToCoordinates(this._items, firstValue, undefinedIfNull(this._itemsVisibleRange));
        };
        DotPaneViewBase.prototype._createRawItemBase = function (time, price, colorer, order) {
            if(order){
                var x = 0;
            }
            return {
        ...
        DotPaneViewBase.prototype._fillRawPoints = function () {
            var _this = this;
            var barValueGetter = this._series.barFunction();
            var newItems = [];
            var colorer = this._series.barColorer();
            this._series.bars().each(function (index, bar) {
                var value = barValueGetter(bar.value);
                var order = bar.order;
                var item = _this._createRawItem(index, value, colorer, order);
                newItems.push(item);
                return false;
            });
            this._items = newItems;
        };
        return DotPaneViewBase;
    }(SeriesPaneViewBase));

    var PairPaneViewBase = /** @class */ (function (_super) {
        __extends(PairPaneViewBase, _super);
        function PairPaneViewBase(series, model) {
            return _super.call(this, series, model, true) || this;
        }
        PairPaneViewBase.prototype._convertToCoordinates = function (priceScale, timeScale, firstValue) {
            timeScale.indexesToCoordinates(this._items, undefinedIfNull(this._itemsVisibleRange));
            priceScale.pointsArrayToCoordinates(this._items, firstValue, undefinedIfNull(this._itemsVisibleRange));
        };
        PairPaneViewBase.prototype._createRawItemBase = function (time, price, colorer, order) {
            return {
                time: time,
                price: price,
                x: NaN,
        ...
        PairPaneViewBase.prototype._fillRawPoints = function () {
            var _this = this;
            var barValueGetter = this._series.barFunction();
            var newItems = [];
            var colorer = this._series.barColorer();
    ...
    var PaneRendererCandlesticks = /** @class */ (function () {
        ...
        PaneRendererCandlesticks.prototype._private__drawWicks = function (ctx, bars, visibleRange) {
            ...
            for (var i = visibleRange.from; i < visibleRange.to; i++) {
                ...
                ctx.beginPath();
                ctx.rect(bar.x, bar.highY, 70, bar.lowY - bar.highY);
                ctx.stroke();
            }
        };
        PaneRendererCandlesticks.prototype._private__drawBorder = function (ctx, bars, visibleRange) {
            ...
            for (var i = visibleRange.from; i < visibleRange.to; i++) {
                ...
                ctx.beginPath();
                ctx.rect(left, top_1, right - left + 1, bottom - top_1 + 1);
                ctx.stroke();
            }
        };
        PaneRendererCandlesticks.prototype._private__drawCandles = function (ctx, bars, visibleRange) {
            ...
            for (var i = visibleRange.from; i < visibleRange.to; i++) {
                ...
                ctx.beginPath();
                ctx.rect(left, top_2, right - left + 1, bottom - top_2 + 1);
                ctx.stroke();
            }
        };
    ...
    var SeriesDotPaneView = /** @class */ (function (_super) {
        __extends(SeriesDotPaneView, _super);
        function SeriesDotPaneView(series, model) {
            var _this = _super.call(this, series, model) || this;
            _this._private__lineRenderer = new PaneRendererDot();
            return _this;
        }
        SeriesDotPaneView.prototype.renderer = function (height, width) {
            this._makeValid();
            var lineStyleProps = this._series.options();
            var data = {
                items: this._items,
        ...
        SeriesDotPaneView.prototype._createRawItem = function (time, price, colorer, order) {
            return this._createRawItemBase(time, price, colorer, order);
        };
        return SeriesDotPaneView;
    }(DotPaneViewBase));


    var SeriesPairPaneView = /** @class */ (function (_super) {
        __extends(SeriesPairPaneView, _super);
        function SeriesPairPaneView(series, model) {
            var _this = _super.call(this, series, model) || this;
            _this._private__lineRenderer = new PaneRendererPair();
            return _this;
        }
        SeriesPairPaneView.prototype.renderer = function (height, width) {
            this._makeValid();
            var lineStyleProps = this._series.options();
            var data = {
                items: this._items,
        ...
        SeriesPairPaneView.prototype._createRawItem = function (time, price, colorer, order) {
            return this._createRawItemBase(time, price, colorer, order);
        };
        return SeriesPairPaneView;
    }(PairPaneViewBase));
    ...
    var SeriesBarColorer = /** @class */ (function () {
        ...
        SeriesBarColorer.prototype.barStyle = function (barIndex, precomputedBars) {
            ...
            switch (targetType) {
                ...
                case 'Dot':
                    return this._private__dotStyle(seriesOptions);
                case 'Pair':
                    return this._private__dotStyle(seriesOptions);
                case 'Area':
        ...
        SeriesBarColorer.prototype._private__dotStyle = function (lineStyle) {
            return __assign(__assign({}, emptyResult), { barColor: lineStyle.color });
        };
    ...
    var Series = /** @class */ (function (_super) {
        ...
        Series.prototype.autoscaleInfo = function (startTimePoint, endTimePoint) {
            ...
            var priceSource = (this._private__seriesType === 'Line' || this._private__seriesType === 'Dot' || this._private__seriesType === 'Pair' || this._private__seriesType === 'Area' || this._private__seriesType === 'Histogram') ? 'close' : null;
            var barsMinMax;
            if (priceSource !== null) {
        ...
        Series.prototype.markerDataAtIndex = function (index) {
            var getValue = (this._private__seriesType === 'Line' || this._private__seriesType === 'Dot' || this._private__seriesType === 'Pair' || this._private__seriesType === 'Area') &&
        ...
        Series.prototype._private__recreatePaneViews = function () {
            ...
            switch (this._private__seriesType) {
                ...
                case 'Dot': {
                    this._private__paneView = new SeriesDotPaneView(this, this.model());
                    break;
                }
                case 'Pair': {
                    this._private__paneView = new SeriesPairPaneView(this, this.model());
                    break;
                }
                case 'Area': {
    ...
    var PaneWidget = /** @class */ (function () {
        ...
        PaneWidget.prototype._private__clearCrosshairPosition = function () {
            // Florin - Never clear last position.
            // return;
    ...
    var seriesItemValueFnMap = {
        ...
        Dot: getLineBasedSeriesItemValue,
        Pair: getLineBasedSeriesItemValue,
    };
            ...
            this._private__sortedTimePoints.forEach(function (time, index) {
                ...
                pointData.mapping.forEach(function (targetData, targetSeries) {
                    ...
                    var seriesUpdate = {
                        index: index,
                        time: time,
                        order: targetData.order ? targetData.order : {},
    ...
    var ChartApi = /** @class */ (function () {
        ...
        ChartApi.prototype.addDotSeries = function (options) {
            if (options === void 0) { options = {}; }
            patchPriceFormat(options.priceFormat);
            var strictOptions = merge(clone(seriesOptionsDefaults), lineStyleDefaults, options);
            var series = this._private__chartWidget.model().createSeries('Dot', strictOptions);
            var res = new SeriesApi(series, this);
            this._private__seriesMap.set(res, series);
            this._private__seriesMapReversed.set(series, res);
            return res;
        };
        ChartApi.prototype.addPairSeries = function (options) {
            if (options === void 0) { options = {}; }
            patchPriceFormat(options.priceFormat);
            var strictOptions = merge(clone(seriesOptionsDefaults), lineStyleDefaults, options);
            var series = this._private__chartWidget.model().createSeries('Pair', strictOptions);
            var res = new SeriesApi(series, this);
            this._private__seriesMap.set(res, series);
            this._private__seriesMapReversed.set(series, res);
            return res;
    ...
    var layoutOptionsDefaults = {
        backgroundColor: '#FFF', // Chart color
        textColor: '#FFFFFF',
        fontSize: 11,
